...
//			return int + dec / factor;
		...
		sector: function (cx, cy, r, startAngle, endAngle) {
			var start = $.wijchart.getPositionByAngle(cx, cy, r, startAngle),
				end = $.wijchart.getPositionByAngle(cx, cy, r, endAngle);
		...
		donut: function (cx, cy, outerR, innerR, startAngle, endAngle) {
			var outerS = $.wijchart.getPositionByAngle(cx, cy, outerR, startAngle),
				outerE = $.wijchart.getPositionByAngle(cx, cy, outerR, endAngle),
				innerS = $.wijchart.getPositionByAngle(cx, cy, innerR, startAngle),
				innerE = $.wijchart.getPositionByAngle(cx, cy, innerR, endAngle),
	...
	$.expr.filter.CLASS = function (elem, match) {
		var className = (!$.wijchart.isSVGElem(elem) ? elem.className :
	...
	$.expr.preFilter.CLASS = function (match, curLoop, inplace, result, not, isXML) {
		...
		for (i = 0, elem = {}; elem; i++) {
			...
			if (elem) {
				className = (!$.wijchart.isSVGElem(elem) ? elem.className :
		...
		// matches .xxxxx or ["xxxxx"] to run over object properties
			...
			_createTooltipEles = function (point, tit, cont, windowCollisionDetection,
				...
				if (o.closeBehavior === "auto") {
					...
					if (content) {
						$.each(content, function (i, c) {
							$(c.node).bind("mouseover.Rtooltip", function (e) {
								_clearTimers();
							}).bind("mouseout.Rtooltip", function (e) {
								_hide(e);
							});
	...
	Raphael.fn.wij = {
		...
		sector: function (cx, cy, r, startAngle, endAngle) {
			return this.path($.wijchart.sector(cx, cy, r, startAngle, endAngle));
		},

		donut: function (cx, cy, outerR, innerR, startAngle, endAngle) {
			return this.path($.wijchart.donut(cx, cy, outerR, innerR,
				startAngle, endAngle));
		},
			...
			if (rotation) {
				if (texts.length > 1) {
					...
					$.each(texts, function (idx, txt) {
						...
						newTxtCenter = $.wijchart.getPositionByAngle(center.x,
	...
	Raphael.el.wijAnimate = function (params, ms, easing, callback) {
		...
		this.animate(params, ms, ease, callback);
		...
		if (shadow && shadow.offset) {
			...
			this.shadow.animate(params, ms, ease, callback);
		}
	};
...
(function ($) {
	...
	$.widget("wijmo.wijchartcore", {
		...
		_init: function () {
			...
			/*
			o.seriesList = $.grep(o.seriesList, function(series, i) {
				var data = series.data;
				if (typeof data === 'undefined' || data === null) {
					return false;
				}
		...
		******************************/
		...
		_paintHeader: function () {
			...
			if (header.text && header.text.length > 0 && header.visible) {
				...
				$.wijchart.addClass($(headerText.node), "wijchart-header-text");
				...
				$.wijchart.addClass($(headerContainer.node), "wijchart-header-container");
		...
		_paintFooter: function () {
			...
			if (footer.text && footer.text.length > 0 && footer.visible) {
				...
				$.wijchart.addClass($(footerText.node), "wijchart-footer-text");
				//update for fixing bug 15884 at 2011/7/5
				...
				$.wijchart.addClass($(footerContainer.node), "wijchart-footer-container");
		...
		_paintLegend: function () {
			...
			if (legend.text && legend.text.length) {
				...
				$.wijchart.addClass($(legendTitle.node), "wijchart-legend-title");
				textStyle = $.extend(true, {}, o.textStyle,
			...
			$.each(tempSeriesList, function (i, series) {
				...
				function drawSeriesLegend(series) {
					var seriesStyle = legend.reversed ?
						seriesStyles[tempSeriesList.length - 1 - idx++] : 
						seriesStyles[idx++],
					...
					if (series.legendEntry && series.display !== "exclude") {
						...
						$.wijchart.addClass($(text.node), "wijchart-legend-text");
						...
						$.wijchart.addClass($(icon.node), "wijchart-legend-icon");
						icon.attr(chtStyle);
			...
			$.wijchart.addClass($(legendContainer.node), "wijchart-legend-container");
			...
			if (self._hasAxes()) {
				...
				if (self.innerState.axisInfo) {
					...
					if (axisOption.x.autoMin && self.axisInfo.x.autoMin) {
						...
						axisOption.x.min = $.wijchart.toOADate(axisOption.x.min);
					}
					if (axisOption.x.autoMax && self.axisInfo.x.autoMax) {
						...
						//if is date time, convert to number.
						axisOption.x.max = $.wijchart.toOADate(axisOption.x.max);
					}
					if (axisOption.y.autoMin && self.axisInfo.y.autoMin) {
						...
						//if is date time, convert to number.
						axisOption.y.min = $.wijchart.toOADate(axisOption.y.min);
					}
					if (axisOption.y.autoMax && self.axisInfo.y.autoMax) {
						...
						//if is date time, convert to number.
						axisOption.y.max = $.wijchart.toOADate(axisOption.y.max);
					}
		...
		_autoPositionCartesianAxis: function (axisInfo, axisOptions, dir) {
			...
			if (origin !== null && self._isDate(origin)) {
				origin = $.wijchart.toOADate(origin);
			}
		...
		_getSortedDataValues: function (axisInfo, axisOptions) {
			...
			$.each(valueLabels, function (idx, label) {
				...
				//	rc.push($.wijchart.toOADate(label));
			...
			try {
				if (unit === 0) {
					...
					if (autoTick) {
						...
						smin2 = $.wijchart.round(self._signedCeiling(smin / unit) * unit,
											tickprec + 1);
						if (smin2 < smax) {
							smin = smin2;
						}
						imax = parseInt($.wijchart.round(smax / unit, 5), 10);
						imin = parseInt($.wijchart.round(smin / unit, 5), 10);
					...
					for (i = 0; i < n; i++) {
						if (round) {
							//vals[i] = $.wijchart.round(smin + i * unit, tickprec + 1);
							if (autoTick) {
								vals[i] = $.wijchart.round(smin + i * unit, tickprec + 1);
							} else {
								vals[i] = smin + i * unit;
		...
		_paintAxis: function (axisOptions, axisInfo) {
			...
			if (axisOptions.visible) {
				...
				$.wijchart.addClass($(ax.node), "wijchart-axis");
		...
		_paintAxisText: function (axisOptions, axisInfo) {
			...
			$.wijchart.addClass($(textElement.node), "wijchart-axis-text");
		...
		_paintMajorMinor: function (max, min, val, tick, unit, tickRect, compass,
			...
			if (isVertical) {
				...
				if (grid.visible) {
					if ((y !== bs.startY && compass === "east") ||
						...
						$.wijchart.addClass($(p.node), "wijchart-axis-gridline");
						p.attr(grid.style);
			...
			if (tick !== "none") {
				...
				$.wijchart.addClass($(tickElement.node), "wijchart-axis-tick");
			...
			if (text !== null) {
				if (labelWidth) {
					...
					$.wijchart.addClass($(txt.node), "wijchart-axis-label");
		...
		_setChartLabel: function (chartLabel, point, angle, calloutStyle) {
			...
			$.wijchart.addClass($(text.node), "wijchart-label-text");
			...
			if (offset && chartLabel.connected) {
				...
				$.wijchart.addClass($(p.node), "wijchart-label-connect");
				p.attr(calloutStyle);
		...
		_getCompassTextPosition: function (compass, box, offset, point, angle) {
			...
			endPoint = $.wijchart.getPositionByAngle(point.x, point.y, offset, angle);
		...
		_bindLegendEvents: function () {
			...
			function bindLegendEvent(i) {
				var fields = self.chartElement.data("fields"),
			...
			if (self.legends) {
				$.each(self.legends, function (index, n) {
					n.click(function (e) {
						bindLegendEvent(index);
					});
				});
			}

			if (self.legendIcons) {
				$.each(self.legendIcons, function (index, n) {
					n.click(function (e) {
						bindLegendEvent(index);
					});
				});
			...
			if (self.legendDots) {
				$.each(self.legendDots, function (index, n) {
					n.click(function (e) {
						bindLegendEvent(index);
					});
				});
		...
		_unbindLiveEvents: function () {
			this._unbindLegendEvents();
		},

		_unbindLegendEvents: function () {
			var self = this;
			if (self.legends) {
				$.each(self.legends, function (index, n) {
					$(n.node).removeData();
		...
		_roundTime: function (timevalue, unit, roundup) {
			...
			//tunit = unit * self._tmInc.day,
				tunit = unit,
				tv = $.wijchart.fromOADate(timevalue),
				th,
				td,
				tx,
				tz;
		...
		_getTimeAsDouble: function (th) {
			...
			*/
			...
			return $.wijchart.toOADate(newDate);
		},
		...
		_getMinMaxValue: function (array) {
			...
			if (self._isDate(val.min)) {
				val.min = $.wijchart.toOADate(val.min);
				val.max = $.wijchart.toOADate(val.max);
			}
		...
		_calculateMajorMinor: function (axisOptions, axisInfo) {
			...
			if (autoMajor) {
				...
				if (isTime) {
					//maxText = $.format($.wijchart.fromOADate(maxData), formatString);
					maxText = 
					Globalize.format($.wijchart.fromOADate(maxData), formatString);
					//minText = $.format($.wijchart.fromOADate(minData), formatString);
					minText = 
					Globalize.format($.wijchart.fromOADate(minData), formatString);
					...
					mx = self._text(-1000, -1000,
						$.wijchart.round(maxData, _prec)).attr(textStyle);
					mn = self._text(-1000, -1000,
						$.wijchart.round(minData, _prec)).attr(textStyle);
